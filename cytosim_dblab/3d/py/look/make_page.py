...
Usage:
    ...
    make_page.py [recursive=0] [table=INT] [width=INT] [height=INT] [OUTPUT] DIRECTORIES

Info:
    
    If `table` is set, a HTML table will be generated, and the value specified will be the
...
def selectFiles(dirpath, files):
    ...
    for f in files:
        ...
        if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.gif') or f.endswith('.svg'):
            images.append(path)
...
def process(dirpath, directories, files):
    ...
    """
    global out, indx, table, recurs
    dirname = dirpath.lstrip('./')
    if table > 0:
        out.write('<td>\n')
    out.write('<h3>\n'+dirname)
    if 'config.cym' in files:
        out.write(' &mdash; <a href="%s/config.cym">config</a>\n' % dirpath);
    if 'movie.mp4' in files:
        out.write(' &mdash; <a href="%s/movie.mp4">movie</a>\n' % dirpath);
    out.write('</h3>\n')
...
def command(args):
    """generates HTML page"""
    global output, out, imsize, table, recurs
    paths = []
    ...
    if table > 0:
        out.write('<table border="0" align="center" cellpadding="3">\n')
        out.write('<tr>\n')
