...
try:
    ...
    sys.stderr.write("could not load necessary python modules on %s\n" % host)
    time.sleep(10000)
    sys.exit()
...
try:
    ...
    err.write("could not load go_sim_lib.py\n")
    sys.exit()
...
def run(exe, base, park, conf, name, preconf, repeat):
    ...
    if not os.path.isdir(base):
        err.write("Error: directory '%s' does not exist\n" % base)
        sys.exit()
    
    if not executable(exe):
        err.write("Error: executable '%s' not found\n" % exe)
        sys.exit()

    if not os.path.isfile(conf):
        err.write("Error: file '%s' does not exist\n" % base)
        sys.exit()
    
    # generate config file:
    if preconf:
        import tempfile
        wdir = tempfile.mkdtemp('', 'go.', '.')
    else:
        wdir = ''
    files = go_sim_lib.make_config(conf, preconf, repeat, wdir)

    if not files:
        err.write("Error: could not generate config files\n")
        sys.exit()
        ...
        if os.path.isdir(park):
            try:
                ...
                with open(s+"/log.txt", "a") as f:
                    f.write("parked    %s\n" % time.asctime())
                sys.stdout.write("Run %s parked in %s\n" % (conf,s))
            except Exception as e:
                err.write("Error: %s\n" % e)
        else:
            sys.stdout.write("Run %s completed in %s\n" % (conf,s))
    ...
    for arg in args:
        if arg.isdigit():
            ...
            if not os.path.isdir(park):
                err.write("Error: `%s' is not a directory\n"%park)
                sys.exit()
        else:
